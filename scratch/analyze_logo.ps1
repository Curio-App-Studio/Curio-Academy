...
Write-Output "Corner pixel (5,5): R=$($corner.R), G=$($corner.G), B=$($corner.B)"
...
Write-Output "Emblem Bounding Box: minX=$minX, maxX=$maxX, minY=$minY, maxY=$maxY"
Write-Output "Emblem Dimensions: width=$($maxX - $minX), height=$($maxY - $minY)"
$bmp.Dispose()
